...
function jLoadDataToday() {
    return $.ajax({
        ...
        type: "POST",
        dataType: "json",
        url: jPath + "&mode=data-today&t=" + (Date.now()),
        beforeSend: function() {
        ...
        success: function(data, status) {
            ...
            $('.' + td[7] + '1').html(addCommas(parseFloat(parseFloat($xdata.AW1) + parseFloat($xdata.AW2)).toFixed(3)));
...
function jLoadDataDump() {
    return $.ajax({
        ...
        type: "POST",
        dataType: "json",
        url: jPath + "&mode=data-dump&t=" + (Date.now()),
        beforeSend: function() {
            $('.loader1').html(jLoader);
        },
        success: function(data, status) {
            ...
            $('.' + td[1] + '2').html(addCommas(data_out_dump.AW1 * ($xdata.AT1 * 1.000)));
            $('.' + td[2] + '2').html($xdata.P1 * 1.00);
            $('.' + td[3] + '2').html(addCommas($xdata.AT2));
            $('.' + td[4] + '2').html(addCommas(data_out_dump.AW2 * ($xdata.AT2 * 1.000)));
            $('.' + td[5] + '2').html($xdata.P2 * 1.00);
            $('.' + td[6] + '2').html(addCommas(parseInt($xdata.AT1) + parseInt($xdata.AT2)));
            $('.' + td[7] + '2').html(addCommas(parseFloat((data_out_dump.AW1 * ($xdata.AT1 * 1.000)) + (data_out_dump.AW2 * ($xdata.AT2 * 1.000))).toFixed(3)));

            let wSum = parseFloat(parseFloat(data_out_dump.AW1 * 1.000) + parseFloat(data_out_dump.AW2 * 1.000) + parseFloat($xdata.AW1 * 1.000) + parseFloat($xdata.AW2 * 1.000));

            $('.' + td[0] + '3').html(addCommas(data_out_dump.AT1 * 1 + $xdata.AT1 * 1));
            let w1 = ((data_out_dump.AW1 * ($xdata.AT1 * 1.000)) + data_out_dump.AW1 * 1.000);
            $('.' + td[1] + '3').html(addCommas(w1));
            $('.' + td[2] + '3').html(addCommas(parseFloat((w1 / wSum) * 100).toFixed(2)));

            $('.' + td[3] + '3').html(addCommas(data_out_dump.AT2 * 1 + $xdata.AT2 * 1));
            let w2 = (data_out_dump.AW2 * ($xdata.AT2 * 1.000)) + data_out_dump.AW2 * 1.000;
            $('.' + td[4] + '3').html(addCommas(w2));
            $('.' + td[5] + '3').html(addCommas(parseFloat((w2 / wSum) * 100).toFixed(2)));

            $('.' + td[6] + '3').html(addCommas(parseInt(data_out_dump.AT1) + parseInt(data_out_dump.AT2) + parseInt($xdata.AT1) + parseInt($xdata.AT2)));
            $('.' + td[7] + '3').html(addCommas(wSum.toFixed(3)));
...
function jLoadDataQIN() {
    return $.ajax({
        ...
        type: "POST",
        dataType: "json",
        url: jPath + "&mode=data-in&t=" + (Date.now()),
        beforeSend: function() {
            $('.loader1').html(jLoader);
        },
        success: function(data, status) {
            ...
            $('.' + td[7] + '4').html(addCommas(parseFloat(parseFloat($xdata.AW1 * 1.000) + parseFloat($xdata.AW2 * 1.000)).toFixed(3)));
...
function jLoadDataQOUT() {
    return $.ajax({
        ...
        type: "POST",
        dataType: "json",
        url: jPath + "&mode=data-out&t=" + (Date.now()),
        beforeSend: function() {
            $('.loader1').html(jLoader);
        },
        success: function(data, status) {
            ...
            $('.' + td[7] + '5').html(addCommas(parseFloat(parseFloat($xdata.AW1 * 1.000) + parseFloat($xdata.AW2 * 1.000)).toFixed(3)));
...
function jLoadDataQ() {
    ...
    return $.ajax({
        type: "POST",
        dataType: "json",
        url: jPath + "&mode=data-q&t=" + (Date.now()),
        beforeSend: function() {
            $('.dataQ-loader').show();
        },
...
function jLoadDataCane() {
    return $.ajax({
        type: "POST",
        dataType: "json",
        url: jPath + "&mode=data-cane&t=" + (Date.now()),
        beforeSend: function() {
