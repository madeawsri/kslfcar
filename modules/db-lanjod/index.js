...
function data_info(data_row,dbdata){
  ...
  //jLog(dbdata[3]);
  $('#mModal').find("input[name='lanjod_name']").val(data_row[1]);
  $('#mModal').find("input[name='lanjod_ip']").val(data_row[2]);
  ...
  $('#mModal').on('shown.bs.modal', function () {
    ...
    $('#mModal').find("input[type='text']" ).change(function() {
  ...
  $('#btn-edit').click(function(){
    ...
    $.ajax({
        ...
        success:function(ret){
          ...
          $(".loading").hide(); 
          if(ret.error){
            $('#mModal').find('.alert-warning.errmsg').html(ret.errmsg);
