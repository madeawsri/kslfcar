  ...
  if(!$err_softpro_has){
      ...
      if(!$params['error']){
        ...
        if(!$params['paid_type']){ // ประเภทคิว "ส่วนตัว"
            ...
            //$fwono = substr(trim($fname[2]),0,10);
            $params['fcucode'] = trim($fname[0]);
            //$fgrpcode = substr(trim($fname[3]),0,7);
            //$params['fgrpcode']  = $fgrpcode;
            //$fname = trim($fname[1]);
          ...
          $queue_remain = $queue_amt -1;
             ...
             if($data_daily){
               ...
               if($params['check_queue_remain'] <= 0 ){  $over_que = "Y";   }
               ...
               $data_daily_insert = array(
                ...
                $params['check']='2';
                $sql_check = " SELECT * FROM `squata_quete_daily` WHERE fyear = '6162' and fgrpcode = '{$params['fgrpcode']}' order by queue_date DESC limit 1 ";
                ...
                if($data_daily){
                  ...
                  if($data_daily['queue_remain'] < -1 ){  $over_que = "Y";  }
                }
                if($data_daily)
                  $data_daily_insert = array(
                    'queue_last_time'=>time()
       ...
       $data_softpro = array(
          "FITEMNO"=> $data_paid['que_id']
          ,"FYEAR"=> "6162" // CUR_YEAR
          ,"FPRDNO"=> "01"
          ,"FVOUDATE"=>  str_replace("-","/",$data_paid['fvoudate'])
          ,"FMOVECODE"=>"19"
          ,"FSUBTYPE"=> $data_paid['cane_type']
          ,"FCONTCODE"=> $data_paid['fcucode']  // FCUCODE
          ...
          ,"FDRIVER"=> $fdriver
          ,"FPDCODE"=> $fpdcode
          ,"FREMARK"=> $data_paid['truck_head'] // truck_head
          ,"FENTDATE"=> date('Y/m/d') 
          ,"FUSER"=> $fuser //'EKK' // demo fix 'EKK'
          ,"FPRJCODE"=> "WG" // 
          ,"FDIVCODE"=>"01" // รหัสโรงงาน
          ,"FQUOTANO"=> $fwono // FWONO
          ,"FATAX"=> "0" 
          ,"FDELBY"=> $fshipvia  // FSHIPVIA
          ,"FQDATE"=> $datein 
          ,"FQTIME"=> $timein
          ,"FUPDDATE"=> $datein 
          ,"FUPDTIME"=> date("H:i:s") 
          ,"FUPDBY"=> $fuser //'EKK' // user demo 'ekk' 
          ,"FQBY"=> $lanjod //  demo
          ,"FTRNNO"=>''
          ,"FGDCODE"=>''
          ,"FINVNO"=>''
          ...
          ,"FQCASHNO"=>''
          ,"FDUMPTIME"=>'00.00'
          ,'FCARDNO'=>($over_que_date <> "")?str_replace('-','',$over_que_date):''
        );
        ...
        if($data_rd01cust){
            ...
            if(strlen($qnext) == 5){  
                ...
                $_dblib->exec_softpro(($gSQL_SoftPro));
