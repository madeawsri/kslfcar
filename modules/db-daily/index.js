    ...
    if(ret){
      ...
      $.ajax({
          ...
          success:function(ret){
            ...
            swal(
              'ลบข้อมูลทั้งหมด !',
              'คุณได้ลบข้อมูลทั้งหมดเรียบร้อยแล้ว.',
              'success'
            );
            table.ajax.reload();
          }
...
function data_info(data_row,dbdata){
  ...
  //jLog(data_row[3].replace(/\//g,'-'));
  $('#mModal').find("input[name='queue_date']").val(data_row[3].replace(/\//g,'-'));

  $('#mModal').find("input[name='queue_amt']").val(data_row[5].replace(",", ""));

  $('#mModal').find("input[name='queue_remain']").val(data_row[6].replace(",", ""));
  ...
  $('#mModal').on('shown.bs.modal', function () {
    ...
    $('#mModal').find("input[name='queue_remain']").focus();
  ...
  $('#btn-del').click(function(e){
      ...
      swal({
        ...
        if(ret){
          ...
          $.ajax({
            type:'POST',
            url  : jPath+"&mode=del&id="+data_row[0],
            dataType: 'json',
            cache: false,
            beforeSend: function () { $(".loading").show(); },
            success:function(ret){
