...
function jAdd(p_carno) {
    ...
    $.ajax({
      ...
      success: function(ret) {
        ...
        if (Object.keys(ret.data_car).length > 0) {
          ...
          getObjModal('.keycode').html(zks[1] + "-" + (ret.data_car.id));
  ...
  getObjModal('#txt_zks').change(function(e) {
    ...
    if (cartype_id == "C3") {
      ...
      // has data hcar
      if (is_value(check_hcarno.check_hcarno)) {
        ...
        getObjModal('.hcarno').val(check_hcarno.check_hcarno).trigger("change").select2('open');
        //getObjModal('.hcarno').removeAttr('readonly').val(check_hcarno.check_hcarno).attr('readonly', 'readonly').trigger("change").select2('open');
