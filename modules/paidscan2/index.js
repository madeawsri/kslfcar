...
function jAdd() {
    ...
    $('#mModal').on('shown.bs.modal', function() {
        ...
        if (getObjModal('#rType').val() == 1) { // รถส่วนตัว
            ...
            if (jCFarm === '') {
                ...
                $('#mModal').find('.alert-warning').hide();
                getObjModal('.paid_type1').hide();
                getObjModal('.div_barcode').hide();
                getObjModal('.div_qoata').show();
                getObjModal('.div_canetype').hide();

                getObjModal('.not_cfarm').show();
        ...
        jSelect2Data_new($('#mModal').find('#center_no'), 'เลือกศูนย์ขนถ่าย', jPathJson, 'center', $('#mModal'));
        ...
        getObjModal('#paid_type').change(function() {
            ...
            if (qType * 1 === 1) {
                ...
                getObjModal('.paid_type0').hide();
                getObjModal('.paid_type1').show();
                getObjModal('.paid_type2').show();
                getObjModal('.div_canetype').show();

                setTimeout(function() {
                    jLog('--focus center ---');
                    getObjModal('#center_no').empty().trigger('change').select2('open');
                }, 1500);
    ...
    getObjModal('#center_no').change(function() {
        var ccode = $(this).val();

        if (ccode === null) return false;


        var qType = getObjModal('#paid_type');
        if (qType.val() * 1 == 3) {
            console.log(ccode, ' : qType ', qType.val());
            jSelect2Data_new($('#mModal').find('#truck_no'), 'เลือกเลขที่สั่งงาน', jPathJson + "&cncode=" + ccode, 'freqno-center', $('#mModal'));
            setTimeout(function() {
                jLog('--focus center ---');
                getObjModal('#truck_no').empty().trigger('change').select2('open');
            }, 1500);
        ...
        try {
            ...
            if (is_value(_data.checkq)) {
                ...
                if (fcucode) {
                    ...
                    getObjModal('.fhead').hide();
                    if (cartype_id == 'C4') {
                        jLog("type_car : " + cartype_id);
                        getObjModal('.fhead').show();
                    }
                } else {
                    jAlertWarning({
    ...
    getObjModal('#rType2').click(function() {
        actQ(3);
    });
    ...
    getObjModal('#fcucode').change(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (checkCarno == 1) { // ตรวจสอบทะเบียนช้ำในลาน
                ...
                } else if (dataQTime.qchktime.checkdate == 0 && ret.isCheckHr) { // ตรวจสอบเวลาชั่งออก หลัง 3 ซม.
                    ...
                    if (data) {
                        ...
                        // 120% for FW T & U
                        if (ret.isCheckPerSent)
                            if (ret.fw_tu['percen_fw'] > parseInt(ret.xpersent))
                                getObjModal('#btn-save').hide();
...
function submitForm() {
    ...
    if ($('#mModal').find('#center_no').select2('data').length > 0)
        var centerno = $('#mModal').find('#center_no').select2('data')[0].text;


    //console.log('Save Q : ', centerno, fcanetype, ftrackno)

    setTimeout(function() {
        $.ajax({
            type: 'POST',
            url: jPath + "&mode=add&fvname=" + fcartype + "&fcanetype=" + fcanetype + "&truckno=" + ftrackno + "&centerno=" + centerno,
            data: formData,
            dataType: 'json',
            cache: false,
            beforeSend: function() {
                    ...
                    if (ret.error) {
                        ...
                        if (ret.status_print == 1) {
                            ...
                            setTimeout(function() {
                                ...
                                $(".fstatus").html('-สถานะการส่งอ้อยชาวไร่-');
                                $('#mModal').find('#frm').show();
                                $('#mModal').find('.print_bill').hide();
                                ...
                                getObjModal('#rType').prop('checked', true);
                            }, 100);
