 ...
 * https://github.com/Clorith/SQLSRV-DataBase-Class
 * @version 0.2.0
 * @license GPLv2
 */
class SqlSrv extends FnBase {
	/**
	 * The last ran query
	 *
	 ...
	 */
	public function __construct( $dbhost,$dbuser, $dbpassword, $dbname,  $dbport = 1433, $build_schema = false ) {
		$this->dbuser       = $dbuser;
	...
	public function db_connect() {
		...
		$this->db = sqlsrv_connect( $this->dbhost , $connectionOptions );

		//$connectionInfo = array( "Database"=>$this->database, "UID"=>$this->username, "PWD"=>$this->password);
		//$this->connect_db = sqlsrv_connect( $this->host, $connectionInfo) or $this->_error();
	 ...
	 * @return void
	 */
	public function insert( $table, $data , $show_sql = 0) {
		$fields = '';
		$values = '';
  ...
  /**
	 ...
	 * @return array|bool
	 */
	public function select( $query, $format = 'object' ) {
		$response = array();
		$request = $this->query( $query );
		if ( $this->has_error() ) {
			$response = false;
